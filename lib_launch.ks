// Common launch library

FUNCTION doPrelaunch {
	//required scripts
	scrReq("lib_body.ks").
	scrReq("lib_node.ks").
...
FUNCTION doLaunch {
	...
	SET autoAscent TO TRUE.	//Use mathy launch profile
	
	//Error checks
	//Is orbit clear of atmosphere or highest peak
	IF tarAlt*(1-orbitError/100)<atmTop {
		scrollPrint("Target orbit apopapsis of "+tarAlt*(1-orbitError/100)+"m  based on",FALSE).
		scrollPrint("orbit error threshold is below atmosphere height",FALSE).
		scrollPrint("of "+atmTop+"m",FALSE).
		scrollPrint(" ",FALSE).
		scrollPrint("Use ctrl+c to cancel program and try again",FALSE).
		scrollPrint(" ",FALSE).
		WAIT UNTIL False.
	}
	...
	FOR p in SHIP:PARTS {
		IF p:MODULES:CONTAINS("LaunchClamp") {
			...
			ELSE IF p:STAGE <> launchClampStage {
				scrollPrint("Not all launch clamps are in a single Stage",FALSE).
				scrollPrint("You will not go to space today...",FALSE).
				scrollPrint(" ",FALSE).
				scrollPrint("Use ctrl+c to cancel program and try again",FALSE).
				scrollPrint(" ",FALSE).
				WAIT UNTIL False.          
	...
	IF ABS(tarInc) < FLOOR(ABS(LATITUDE)) OR ABS(tarInc) > (180 - CEILING(ABS(LATITUDE))) {
		scrollPrint("Desired inclination impossible. ",FALSE).
		scrollPrint("Magnitude must be larger than or equal to the ",FALSE).
		scrollPrint("current latitude of "+ROUND(LATITUDE,1)+" deg",FALSE).
		scrollPrint(" ",FALSE).
		scrollPrint("Use ctrl+c to cancel program and try again",FALSE).
		scrollPrint(" ",FALSE).
		WAIT UNTIL False.
...
FUNCTION activeEngineInfo {
	...
	PRINT "  Alt.   Landed |  Dist.  Landed | Incl." AT (0,4).
	PRINT "  [km]          |  [km]          | [deg]" AT (0,5).

	PRINT "Apoap.          |Periap.         |  TWR" AT (0,7).
	PRINT " [km]    Landed | [km]    Landed |Max TWR" AT (0,8).
